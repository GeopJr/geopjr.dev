module GeopJr::Watcher
  ...
  private def reload_blog(event : FileWatcher::Event)
    if event.type.deleted? # when a blog file is deleted
      found_blog = GeopJr::BLOG_POSTS.index { |x| x.filename == Path[event.path].basename(".md") }
      if found_blog
        ...
        pages_to_reload = [
          "/blog/#{GeopJr::BLOG_POSTS[found_blog].filename}",
        ...
        GeopJr::BLOG_POSTS.delete_at(found_blog)
        pages = GeopJr::BlogPagination.new(GeopJr::BLOG_POSTS)
        pages.write
      ...
      unless entry.nil?
        ...
        found_blog = GeopJr::BLOG_POSTS.index { |x| x.filename == entry.filename }

        pagination = nil
        if found_blog.nil? # if it's a new blog post / added
           # add it in the front and regenerate just the first pagination page
          GeopJr::BLOG_POSTS.unshift(entry)
          pagination = GeopJr::BlogPagination.new(GeopJr::BLOG_POSTS)
          ...
          GeopJr::BLOG_POSTS[found_blog] = entry
          pagination = GeopJr::BlogPagination.new(GeopJr::BLOG_POSTS)
  ...
  def watch
    ...
    FileWatcher.watch(
      ...
      when .starts_with?(Path["data"].to_s)
        ...
        when "icon_names"
          GeopJr::BlogPagination.new(GeopJr::BLOG_POSTS).write
      ...
      when .starts_with?(GeopJr::CONFIG.paths[:static].to_s)
        GeopJr.move_static_files
        GeopJr.remove_underscores
        GeopJr::Server.request_reload
      end
    end
  end
